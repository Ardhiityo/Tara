    ...
    <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden p-8 w-full">
        <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
            <div class="w-full md:w-1/2">
                <form class="flex items-center" method="GET" action="{{ route('category.index') }}">
                    ...
                    @foreach ($categories as $category)
                        <tr class="border-b dark:border-gray-700">
                            ...
                            <td class="px-4 py-3 flex items-center justify-end">
                                <button id="{{ $category->slug }}-dropdown-button"
                                    data-dropdown-toggle="{{ $category->slug }}-dropdown"
                                ...
                                <div id="{{ $category->slug }}-dropdown"
                                    class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                    <ul class="py-1 text-sm text-gray-700 dark:text-gray-200"
                                        aria-labelledby="{{ $category->slug }}-dropdown-button">
                                        <li>
                                    ...
                                    <div class="py-1 flex justify-center">
                                        <form action="{{ route('category.destroy', ['category' => $category->slug]) }}"
                                            method="post">
                                            @csrf
                                            @method('DELETE')
                                            <button
